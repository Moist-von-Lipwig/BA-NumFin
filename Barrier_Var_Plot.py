...
Barrier = 85
Exercise_Time = 1
dimensions = 32  # The simulations got dimensions + 1 many timepoints
...
for N in Paths:
    ...
    MC_Solutions[i], MC_Variances[i] = OpPrAp.Barrier_Opt_Approx(Price_Process, Strike, Barrier, Interest, Exercise_Time,
                                                               'Call', 'Down', 'In')
    ...
    Price_Process = OpPrAp.PriceProcesses(Start_Price, Interest, Volatility, Timepoints_for_PP, Brownian_Motion)
    # Calculate payoff for each brownian motion and estimate mean and variance
    QMC_Hlt_Solutions[i], QMC_Hlt_Variances[i] = OpPrAp.Barrier_Opt_Approx(Price_Process, Strike, Barrier, Interest, Exercise_Time,
                                                               'Call', 'Down', 'In')
    ...
    Price_Process = OpPrAp.PriceProcesses(Start_Price, Interest, Volatility, Timepoints_for_PP, Brownian_Motion)
    # Calculate payoff for each brownian motion and estimate mean and variance
    QMC_Fre_Solutions[i], QMC_Fre_Variances[i] = OpPrAp.Barrier_Opt_Approx(Price_Process, Strike, Barrier, Interest, Exercise_Time,
                                                               'Call', 'Down', 'In')
    ...
    Price_Process = OpPrAp.PriceProcesses(Start_Price, Interest, Volatility, Timepoints_for_PP, Brownian_Motion)
    # Calculate payoff for each brownian motion and estimate mean and variance
    rQMC_1Hlt_Solutions[i], rQMC_1Hlt_Variances[i] = OpPrAp.Barrier_Opt_Approx(Price_Process, Strike, Barrier, Interest, Exercise_Time,
                                                               'Call', 'Down', 'In')
    ...
    for shift in range(Number_Shifts):
        ...
        Price_Process = OpPrAp.PriceProcesses(Start_Price, Interest, Volatility, Timepoints_for_PP, Brownian_Motion)
        # Calculate payoff for each brownian motion and estimate mean and variance
        shifted_Values[shift], shifted_Variance[shift] = OpPrAp.Barrier_Opt_Approx(Price_Process, Strike, Barrier, Interest, Exercise_Time,
                                                               'Call', 'Down', 'In')
    ...
    Price_Process = OpPrAp.PriceProcesses(Start_Price, Interest, Volatility, Timepoints_for_PP, Brownian_Motion)
    # Calculate payoff for each brownian motion and estimate mean and variance
    rQMC_Fre_Solutions[i], rQMC_Fre_Variances[i] = OpPrAp.Barrier_Opt_Approx(Price_Process, Strike, Barrier, Interest, Exercise_Time,
                                                               'Call', 'Down', 'In')
...
plt.title("Varianzrate für die Down-In Barrier Call Option")
plt.xlabel("Anzahl Simulationen")
plt.ylabel("Geschätzte Varianzen")
plt.grid(axis='y')
